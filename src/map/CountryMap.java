...
public class CountryMap extends PApplet {
	...
	private static final Logger logger = Logger.getLogger(CountryMap.class);
	...
	private MarkerManager<NamedMarker> countryMarkMan; // todo: Till vragen over
														// marker manager
														// (vooral
														// map.addMarkerManager,
														// geen correcte
														// generics)
	...
	public void setup() {
		size(1040, 720); // , GLConstants.GLGRAPHICS); //no bug with markers
							// going outside of map area if this is used
		frameRate(30);
		...
		edgeMarkMan = new MarkerManager<EdgeMarker>();// Generics in
														// markermanager, but
														// not in
														// map.addMarkerManager,
														// cause fuck you.
	...
	private void setupGUI() {
		...
		conflist = cp5.addListBox("Conferences").setPosition(50, 50).setSize(120, 120).setBarHeight(15)
				.setItemHeight(15);
	...
	private void addAllEdgeMarkers() {
		...
		while (rs.hasNext()) {
			...
			logger.debug(String.format("Common papers for %s to %s:%d\n", countryName, otherCountryName, coopCount));
		}
	}
	...
	private void addEdgeMarkers(String confAcronym) {
		...
		while (rs.hasNext()) {
			...
			m.setStrokeWeight(coopCount);
			edgeMarkMan.addMarker(m);
			logger.debug(String.format("Common papers for %s to %s:%d for conf:%s\n", countryName, otherCountryName, coopCount,
					confAcronym));
		}
	}
	...
	public void mouseMoved() {
		...
		for (EdgeMarker m : edgeMarkMan.getMarkers()) {
			if (edgeHitMarkers.contains(m)) {
				...
				// ((NamedMarker)m.getM1()).getName(),
				// ((NamedMarker)m.getM2()).getName());
				edgeMarked = true;
			} else {
				m.setSelected(false);
			}
